...
const FIXTURE_URL = new URL(
  '../../../docs/workstreams/tetris-t37-endgame/fixtures/t37/endgame-v3-intro-03.json',
  import.meta.url,
);
...
const intro02 = ENDGAME_V3_INTRO_DRAFTS[1]!;
const draft = ENDGAME_V3_INTRO_DRAFTS[2]!;
...
describe('Endgame v3 non-published Intro-03 authoring entry', () => {
  ...
  it('rebuilds one unique legal four-row support lesson', () => {
    ...
    expect(rows.slice(-4).map((row) => [...row].filter((cell) => cell === '.').length))
      .toEqual([3, 1, 1, 3]);
    expect([...ENDGAME_DEFINITIONS, intro01, intro02].every(({ seed }) => seed !== draft.seed)).toBe(true);
    for (const other of [...ENDGAME_DEFINITIONS, intro01, intro02]) {
  ...
  it('is canonical schema 8 and rebuilds every frozen hash', () => {
    ...
    expect(new TextEncoder().encode(fixtureBytes)).toHaveLength(1604);
    expect(sha256(fixtureBytes)).toBe('c02d6e5d2ed0d6f4756da2ab439c9bd515f3658ffa536194a1a1c21a33c191e4');
  ...
  it('proves both early-divergent routes build support before spanning it', () => {
    ...
    expect(stateHash(routes[1]!.replay.state)).toBe('911980bc');
...
describe.runIf(RUN_EXACT)('Endgame v3 Intro-03 exact certificate', () => {
