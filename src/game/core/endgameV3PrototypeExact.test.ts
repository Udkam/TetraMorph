...
const FIXTURE_URL = new URL(
  '../../../docs/workstreams/tetris-t37-endgame/fixtures/t37/endgame-v3-ten-row-prototype.json',
  import.meta.url,
);
...
function canonicalBaseBytes(): string {
  return JSON.stringify(getEndgameDefinition(BASE_ID));
}

function prototypeDefinition(): EndgameDefinition {
  const source = getEndgameDefinition(artifact.definition.baseId);
...
function startedState(definition: EndgameDefinition) {
...
function expectBaseUnchanged(reference: EndgameDefinition, bytes: string): void {
  const after = getEndgameDefinition(BASE_ID);
...
describe('Endgame v3 non-published ten-row prototype artifact', () => {
  it('is one canonical UTF-8/LF JSON line with the exact frozen schema and provenance', () => {
    ...
    expect(artifact).toMatchObject({
      artifactVersion: 't37-endgame-v3-ten-row-prototype-v1',
  ...
  it('replays the literal setup and starts the independent gameplay bag without mutating the base', () => {
    const base = getEndgameDefinition(BASE_ID);
    ...
    const setupBoard = replayEndgameSetup(definition.setup);
    ...
    expect(endgameRouteLockLowerBound(started)).toBe(artifact.route.lowerBounds[0]);
  ...
  it('reconstructs every lock through exhaustive landings and replays every settled prefix', () => {
    ...
    const lowerBounds = [endgameRouteLockLowerBound(current)];

    for (const expectedSignature of artifact.route.lockSignatures) {
      const matching = exhaustiveEndgameLandings(current).filter(({ lock }) => (
      ...
      commandStream += encodeEndgameRoute(landing.commands);
      ...
      remainingTargetCounts.push(current.endgameTargetCells.length);
      lowerBounds.push(endgameRouteLockLowerBound(current));

      const prefixReplay = replayEndgameRouteForDefinition(definition, commandStream);
      expect(encodeEndgameRoute(prefixReplay.commands)).toBe(commandStream);
      expect(prefixReplay.locks.map(({ signature }) => signature)).toEqual(signatures);
      expect(prefixReplay.state.lines).toBe(current.lines);
      expect(prefixReplay.state.endgameTargetCells).toHaveLength(current.endgameTargetCells.length);
    }

    expect(commandStream).toBe(artifact.route.commandStream);
    expect(metricsForEndgameRoute(commandStream)).toEqual(artifact.route.metrics);
    ...
    const replay = replayEndgameRouteForDefinition(definition, artifact.route.commandStream);
    expect(encodeEndgameRoute(replay.commands)).toBe(artifact.route.commandStream);
...
describe.runIf(RUN_EXACT)('Endgame v3 ten-row exact certificate', () => {
  it('exhausts every shorter public-control depth without beam or state caps', () => {
    ...
    expect(certificate && encodeEndgameRoute(certificate.replay.commands)).toBe(
      artifact.route.commandStream,
    );
