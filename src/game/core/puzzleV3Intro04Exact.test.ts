...
const [intro01, intro02, intro03, draft] = PUZZLE_V3_INTRO_DRAFTS;
const OPENING_X = 4;
...
function openingColumns(state: GameState): number[] {
  const deepestTargetY = state.puzzleTargetCells.reduce((deepest, cell) => Math.max(deepest, cell.y), -1);
...
function routeEvidence(route: string) {
  ...
  for (const lock of replay.locks) {
    before.push(current);
    const matches = exhaustivePuzzleLandings(current).filter(({ lock: candidate }) => (
  ...
  return {
    ...
    remaining: after.map((state) => state.puzzleTargetCells.length),
    openings: after.map(openingColumns),
    hashes: after.map(stateHash),
  };
}

describe('Puzzle v3 non-published Intro-04 authoring entry', () => {
  it('appends one deeply frozen draft while preserving the live and prior draft baselines', () => {
    const liveBytes = JSON.stringify(getPuzzleDefinition('t3r-cascade-06'));
    const serialized = JSON.stringify(draft);
    expect(PUZZLE_DEFINITIONS).toHaveLength(50);
    expect(PUZZLE_V3_INTRO_DRAFTS).toHaveLength(4);
  ...
  it('replays seven legal zero-clear drops into one admitted four-row corridor', () => {
    ...
    expect(rows.slice(-4).map((row) => [...row].filter((cell) => cell === '.').length))
      .toEqual([5, 2, 3, 2]);
    expect(openingColumns(replayPuzzleRouteForDefinition(draft!, 'S').state)).toEqual([OPENING_X]);
    const prior = [...PUZZLE_DEFINITIONS, intro01!, intro02!, intro03!];
  ...
  it('keeps one canonical schema-8 artifact and rebuilds every frozen hash', () => {
    ...
    expect(new TextEncoder().encode(fixtureBytes)).toHaveLength(1477);
    expect(sha256(fixtureBytes).toUpperCase()).toBe('EC17469F6D7F48CBBFB8A2863D24C79D3232B10DAD851204D52B8BF5D69EFB1A');
  ...
  it('proves the local clear and both lock-2-divergent corridor migrations', () => {
    ...
    const expected = [
      {
        ...
        remaining: [20, 20, 5, 0],
        openings: [[4], [4], [0], []],
        hashes: ['4abbf4a4', '4e6c90e5', '27e8cead', '98105716'],
        migrated: 0,
      },
      {
        ...
        openings: [[4], [4], [1], []],
        hashes: ['4abbf4a4', 'fffb68f6', '37e95b7c', 'e8e07748'],
        migrated: 1,
      },
    ] as const;
    for (const item of expected) {
      ...
      expect(item.evidence.replay.state.puzzleCompletion).toBe('finished');
    }
    expect(primary.after[0]!.puzzleCompletion).toBe('active');
    expect(primary.after[0]!.puzzleTargetCells.length).toBeLessThan(initial.puzzleTargetCells.length);
    expect(stateHash(primary.replay.state)).toBe(artifact.finalStateHash);
    expect(stateHash(alternative.replay.state)).toBe('e8e07748');
...
describe.runIf(RUN_EXACT)('Puzzle v3 Intro-04 exact certificate', () => {
