...
import { __reverseTest as reverse } from './search-endgame-v3-prototype.mjs';
const toolPath = fileURLToPath(new URL('./search-endgame-v3-prototype.mjs', import.meta.url));
...
try {
  const endgames = await import(pathToFileURL(fileURLToPath(new URL('../src/game/core/endgames.ts', import.meta.url))));
  const definition = endgames.getEndgameDefinition('t3r-shaft-01');
  const coreBoard = endgames.replayEndgameSetup(definition.setup);
  ...
  assert.deepEqual(endgames.replayEndgameSetup({ seed: coreCandidate.result.seed,
