...
The v2 standalone test is accepted through initial candidate `71f0a54` and gap repair
`caba9d0`; final test SHA-256 is
`617A31CA0B3D442E1A66AAC686A579A25CB3D34E90BC5F949AD1106690511994`.
It stays within 497 lines and passes 145 checks. Fresh independent QA confirms all five
explicit repair attacks and reports `P0 0 / P1 0 / P2 0 / P3 0 / GAP 0`. Final v1/v2
standalones, typecheck, the complete `430 passed / 10 skipped` suite, and the 767-module
...
Shard 2 is accepted with the same independent evidence; the three-entry cumulative
...
across live and draft definitions. For every live definition,
`comparePuzzleTopologies(live, draft)` must report `exactMatch=false`,
`topologyMatch=false`, and `nearTopology=false`; a different gameplay seed or behavior hash
alone is not evidence of a rebuild. The whole-library audit is not asserted empty because
the unchanged live 50 already contain ten historical near-candidate pairs. The workstream
log records the two-stage teaching review, including the new gap vector and why the optimal
`[1,2]` release sequence is readable.
...
Independent read-only contract review accepts `628f66d..dc035a3` with
`P0 0 / P1 0 / P2 0 / P3 0`. It confirms that ordinary cells plus anchors cannot form
an initial full row in canonical or override mode, all 38 historic behavior hashes are
literal inputs, structural validation remains distinct from per-anchor replay evidence,
...
### Stage F3A structural-validator acceptance (2026-08-10)
...
- Post-source typecheck passes. Focused definition, behavior-baseline, and campaign
  tests pass `3 files / 16 tests`. Independent read-only QA repeats those commands,
  independently matches all 38 hashes, verifies the frozen library/order/seed/route
  surface, and reports `P0 0 / P1 0 / P2 0 / P3 0` for `da264f4..b30ee1f`.
...
- The second equal round replays cursor 312,500 and gives the same 32 shards another
  312,500 new probes each. All remain `budget-exhausted` at cursor 625,000 with the
  unchanged domain/queue digests and zero candidates, naturally complete shards, or
  RSS stops. This invocation replays 10,000,000 probes and adds 10,000,000 new probes
  for 20,000,000 physical attempts; cumulative new coverage across both fair rounds is
  20,000,000. Replay-inclusive accepted/failed/trie-node counts are
  `582,850 / 581,851 / 337,031`; the ordered 32-file manifest digest is
...
- The fourth equal round gives every shard a fourth identical increment. All 32 remain
  `budget-exhausted` at cursor 1,250,000 with zero candidates, complete shards, or RSS
  stops. It replays 30,000,000 probes and adds 10,000,000 new probes for 40,000,000
  physical attempts; cumulative new coverage is 40,000,000. Replay-inclusive
  accepted/failed/trie-node counts are `1,164,487 / 1,162,739 / 337,031`; the ordered
  `filename:SHA256` LF-joined manifest digest is
...
- The exact cells of a settled placement that locks onto an anchor-supported chain are
  tracked as one supported unit; lower clears cannot pull its off-column cells downward
  or tear its shape, and adjacent same-material placements are not inferred into it.
- A tracked support cell removed by the current clear is discarded before collision
  boundaries resolve, so it cannot leave a phantom floor behind.
- Canonical Puzzle target coordinates and supported placement coordinates follow the
  same transform as the board.
...
4. **Survival stone reads through mass and light, never decals.** Bedrock becomes one
   flat-topped vertical cave wall whose contact plane aligns exactly with the board grid.
   The single slab with a few broad planes, the regular low-poly fan, and the later
   dry-stacked masonry courses are all rejected. The replacement uses one continuous
   wall body plus deterministic irregular relief regions at several scales. Those regions
   cross row and column boundaries, share edges without dark mortar outlines, stay close
   in value, and gain weight through vertical shading. They cannot repeat at cell cadence,
   form horizontal courses, expose a `10 × N` grid, dominate the wall as giant facets, or
   add a framed exterior. A narrow top-edge lift preserves the flat collision plane. No
   cracks, chips, dots, stickers, wood grain, stroked veins, brick joints, or per-cell
   ornament may imitate texture; the rocky read must come from continuous relief,
   varied scale, directional light, and depth. Falling
...
16. **The Puzzle header moves down to meet the gallery without lifting the gallery.**
    The `返回首页 / Back to home` control, centred `TetraMorph` wordmark, breathing gap,
    and gallery form one content-height stack that is vertically centred as a unit.
    Desktop keeps 16 px between header and gallery; portrait and short-landscape keep
    12 px. The gallery stays near its established viewport position and dimensions;
...
17. **The Survival bedrock must read as an exposed cavern wall.** Its contact remains
    perfectly level for collision truth, while the visible face is one continuous mass
    with small and medium irregular relief regions crossing all logical cell boundaries.
...
- Browser proof covers both leave actions and keyboard selection; all four Settings
  ...
  landscape with complete values; Survival countdown, a flat-contact continuous cavern
  wall with cross-grid relief and no slab/facet fan/masonry courses, a steady column arrow,
  one/two square stones, 4x cadence, and an atomic
...
immutable attempt, and fixes Base64/checkpoint-tip semantics; independent review then rejects
...
Static implementation then identifies that its CLI descriptor omitted the outer mode responsible
for attempt/task ownership. R10 adds explicit `--outer`, shared `--root/--input/
--domain-json-base64/--expect-head` grammar, and task-only worker arguments. Commit and obtain
two independent all-zero R10 document reviews before external source authoring. No validator
