...
import { ITEMS, SEEDS, assertRuntimeInputBinding, root } from './evidence-contract.mjs';
...
/** @type {any} */
const audit = { schema: 'tetramorph.t37.material-semantic.v2', generatedAt: new Date().toISOString(), origin, browser: await browser.version(), runtimeInput, cases: {}, errors: [] };
...
try {
  for (const item of /** @type {Array<'freeze'|'bomb'|'multiplier'|'collapse'>} */ (ITEMS)) {
    ...
    } finally { await material.context.close(); }
    ...
    } finally { await activated.context.close(); }
  ...
  if (JSON.stringify(runtimeInputAfter) !== JSON.stringify(runtimeInput)) throw new Error('Runtime input binding changed during semantic capture.');
