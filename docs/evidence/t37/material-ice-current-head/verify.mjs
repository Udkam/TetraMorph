...
check(initialIceEvents.length === 2
  && deepEqual(initialIceEvents.map((/** @type {any} */ event) => event.sequence), ice.runtimeResponses.map((/** @type {any} */ response) => response.sequence))
  && initialIceEvents.every((/** @type {any} */ event) => event.sequence <= browser.hmr?.marker?.eventSequence)
  && lateIceEvents.every((/** @type {any} */ event) => event.sequence > browser.hmr?.marker?.eventSequence),
'Ice initial Freeze response window isolated from HMR');
