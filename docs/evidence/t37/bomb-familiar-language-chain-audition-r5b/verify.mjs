...
check(manifest.byteContract.manifestSelfHash === 'excluded' && manifest.byteContract.text === 'UTF-8 no-BOM LF-only', 'manifest byte/self-hash contract');
...
const browser = JSON.parse(blob(generated, prefix + 'browser-report.json').toString('utf8'));
...
check(browser.stopped.audio.liveContexts === 1 && browser.stopped.audio.eventSources === 0 && browser.stopped.renderer.tickerOwners === 1 && browser.stopped.canvasCount === 1 && browser.stopped.pendingTimers === 0 && browser.stopped.renderer.frameCallbacks === 0, 'browser reusable stop owners');
check(browser.disabled.enabled === false && browser.disabled.audio.eventSources === 0 && browser.disabled.audio.liveContexts === 1 && browser.disabled.pendingTimers === 0 && browser.disabled.renderer.frameCallbacks === 0, 'browser reusable disable owners');
check(browser.enabled.enabled === true && browser.enabled.audio.liveContexts === 1 && browser.enabled.renderer.tickerOwners === 1, 'browser re-enable no duplication');
check(browser.mobileTerminal.audio.liveContexts === 0 && browser.mobileTerminal.renderer.tickerOwners === 0 && browser.mobileTerminal.canvasCount === 0 && browser.mobileTerminal.listenerCount === 0 && !browser.mobileTerminal.qaGlobals, 'browser terminal destroy');
for (const kind of ['stop', 'disable']) {
  const race = browser.races[kind];
  check(race.playCount === 0 && race.pendingTimers === 0 && race.renderer.frameCallbacks === 0 && race.audio.eventSources === 0 && race.audio.liveContexts === 1 && race.renderer.tickerOwners === 1, `browser delayed ${kind} generation cancellation`);
}
...
check(browser.races.pagehide.playCount === 0 && browser.races.pagehide.audio.liveContexts === 0 && browser.races.pagehide.renderer.tickerOwners === 0 && browser.races.pagehide.listenerCount === 0 && !browser.races.pagehide.qaGlobals, 'browser delayed pagehide generation cancellation');
check(browser.races.hmr.old.playCount === 0 && browser.races.hmr.old.audio.liveContexts === 0 && browser.races.hmr.old.renderer.tickerOwners === 0 && browser.races.hmr.old.listenerCount === 0 && !browser.races.hmr.old.qaGlobals, 'browser delayed HMR old generation cancellation');
check(browser.races.hmr.fresh.audio.liveContexts === 1 && browser.races.hmr.fresh.renderer.tickerOwners === 1 && browser.races.hmr.fresh.canvasCount === 1 && browser.races.hmr.fresh.listenerCount === 3 && browser.races.hmr.fresh.qaGlobals, 'browser delayed HMR one fresh owner set');
